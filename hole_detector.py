...
class HoleDetectorConfig:
    ...
    # 가깝다(튀어나옴). "정상(flush)"은 recess가 0 근처일 때뿐이고, 양쪽 어느
    # 방향으로든 벗어나면 "덜 박힘"이다 - 아직 설계 깊이만큼 다 안 들어가
    # 오목한 굴곡이 남아있어도(recess 양수), 표면 밖으로 튀어나와도(recess
    # 음수) 둘 다 "제대로 안 박힘"이라는 뜻이라서.
    stud_seating_tolerance_mm: float = 1.0  # |recess|가 이 안이면 표면과 거의 같은 높이(정상)로 봄
    stud_partial_max_depth_mm: float = 3.0  # 설계상 홀 깊이 상한 - 이보다 깊게 나오면(예: 빈 홀 오인) 신뢰 못 함 -> 원인불명
    ...
    # 덜 박힌 스터드는 구멍 벽에 가려 은색이 아주 작게(가끔 그늘져) 보여서, 위
    # 블롭 탐지의 면적/밝기 기준을 못 넘긴다 - 그래서 빈 홀 탐지와 같은 depth
    # 굴곡 후보 중 "안에 은색이 있는" 것도 스터드 후보로 추가로 잡는다
    # (`detect_stud_states` 참고). 처음엔 "은색 픽셀이 하나라도 있으면" 기준을
    # 썼더니 반사광 노이즈 한두 픽셀도 스터드로 오탐지하는 문제가 실측으로
    # 확인돼서, 최소 면적 기준을 둔다.
    stud_partial_min_silver_area_mm2: float = 1.0  # 이보다 넓게 은색이 뭉쳐 있어야 노이즈가 아닌 진짜 스터드로 인정
...
class HoleDetector:
    ...
    def classify_stud_state(
        ...
        - 전체적으로 얼마나 안쪽/바깥쪽에 있는지(seating_offset_mm)는
          ...
          (`stud_partial_max_depth_mm`)보다 크면 신뢰할 수 없는 값(예: 빈 홀을
          스터드로 착각)으로 보고 "원인불명" 처리한다.
